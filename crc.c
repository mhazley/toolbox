 ...
 *
 * crc.c
 * CRC8 and CRC32 functions, both fast (using L.U.T, requires memory) or slow
 * (no L.U.T required).
 *
 */
...
void crc32_fast( const void* buffer, size_t buffer_length, uint32_t* chksum )
{
	const uint8_t *p = buffer;
    uint8_t padding_length = 0U;

    /* Zero padded to word length */
    if( ( buffer_length % sizeof(uint32_t) ) != 0 )
    {
        padding_length = (uint8_t)( ( sizeof(uint32_t) ) - ( buffer_length % sizeof(uint32_t) ) );
    }
    ...
    while (padding_length--)
    	crc = crc32_table[(crc ^ 0x00) & 0xFF] ^ (crc >> 8);
...
void crc32_slow( const void* buffer, size_t buffer_length, uint32_t* chksum )
{
    int i;
    const uint8_t *p = buffer;
    uint8_t padding_length = 0U;
...
void crc8_fast( const void* buffer, size_t buffer_length, uint8_t* chksum )
{
    const uint8_t *p = buffer;
    uint8_t crc = 0xFF;

    while (buffer_length--)
       crc = crc8_table[(crc ^ *p++) & 0xFF] ^ (crc >> 8);
...
void crc8_slow( const void* buffer, size_t buffer_length, uint8_t* chksum )
{
    int i;
    const uint8_t *p = buffer;
	uint8_t crc = 0xFF;
    uint8_t mask;
    uint8_t POLYNOMIAL = 0xE0;
...
{
    const uint8_t crc32Data[] = { 0x01, 0x02, 0x08, 0x12, 0x34, 0x56, 0x78, 0x00 };
    const uint8_t crc8Data[]  = { 0xD2, 0x04 };

    uint32_t crc32Fast     = 0;
    uint32_t crc32Slow     = 0;
    uint8_t  crcEightFast  = 0;
    uint8_t  crcEightSlow  = 0;

    crc32_fast( crc32Data, sizeof(crc32Data), &crc32Fast );
    crc32_slow( crc32Data, sizeof(crc32Data), &crc32Slow );
    crc8_fast ( crc8Data,  sizeof(crc8Data),  &crcEightFast );
    crc8_slow ( crc8Data,  sizeof(crc8Data),  &crcEightSlow );

    printf( "CRC-32 Fast: 0x%08X\n", crc32Fast );
    printf( "CRC-32 Slow: 0x%08X\n", crc32Slow );
    printf( "CRC-8  Fast: 0x%02X\n", crcEightFast );
    printf( "CRC-8  Slow: 0x%02X\n", crcEightSlow );
